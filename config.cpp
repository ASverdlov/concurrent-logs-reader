...
void Config::ParseOptions(int argc, char* argv[]) {
  ...
  optional_desc.add_options()
      ...
      ("output_file", po::value<std::string>(&OutputFile)->default_value("agr.txt"), "output file")
  ;

  po::options_description required_desc("Required options");
  required_desc.add_options()
      ("path", po::value<std::string>(&Directory)->required(), "directory with logs")
      ("jobs", po::value<unsigned>(&JobsNum)->required(), "threads number")
      ("count", po::value<unsigned>(&FilesNum)->required(), "logs number")
  ;
